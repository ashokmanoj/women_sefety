...
const PanicMode = () => {
  ...
  const { active, recording } = useSelector(
    (state: RootState) => state.emergency.panicMode
  );
  ...
  const handlePanicToggle = () => {
    dispatch(togglePanicMode());
  };

  // Panic Button Component
  return (
    <div
      className={`fixed bottom-24 right-8 flex flex-col items-end space-y-4`}
    >
      {active && (
